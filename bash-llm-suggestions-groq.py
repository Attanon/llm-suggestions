...
def generate_shell_script(client, buffer, os_info):
    system_message = f"""You are a bash shell expert on {os_info}. Please write a complete shell script that solves the given problem.
...
def main():
    ...
    system_message = f"""You are a bash shell expert on {os_info}, please write a BASH command that solves my problem.
    ...
    if mode == 'explain':
        system_message = f"""You are a bash shell expert on {os_info}, please briefly explain how the given command works. Be as concise as possible. Use Markdown syntax for formatting. If there are any {os_info}-specific considerations, mention them.
